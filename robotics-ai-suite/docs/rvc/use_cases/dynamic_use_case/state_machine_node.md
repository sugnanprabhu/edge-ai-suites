...
UR5e tracking an erratically moving object, optionally picking it up and placing at
...
showcase. The only mandatory option is `robot_ip` which specifies where the real robot
...
A [MoveIt 2](https://moveit.picknik.ai/main/index.html) based motion controller has been implemented showing a dynamic
...
For most of these parameters, we refer to the official [MoveIt 2 Servo tutorial](https://moveit.picknik.ai/humble/doc/examples/realtime_servo/realtime_servo_tutorial.html).
...
These collision boxes will be used if the parameter
...
The file `waypoint.yaml` exposes the waypoints the dynamic and static use cases use to navigate the robot:
...
How to derive these numbers:

ROS2 is using a different coordinate system than the Universal Robot teach pendant. To convert
the two, here is the conversion:

![UR External Control](../../images/html/convertWaypoint.png)
