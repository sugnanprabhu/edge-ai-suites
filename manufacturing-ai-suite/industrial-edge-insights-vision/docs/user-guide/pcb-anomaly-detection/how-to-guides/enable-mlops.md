...
If not available, you can simulate this by downloading the sample model from edge-ai-resources repository [here](https://github.com/open-edge-platform/edge-ai-resources/blob/main/models/FP16/pcb-anomaly-detection.zip). Once downloaded, extract to `/tmp/models` directory.
...
10. Start a new pipeline with this new model. Before that modify the payload.json to use this new model in `apps/pcb-anomaly-detection/payload.json`. Notice the model path in the payload has changed to the new model.
