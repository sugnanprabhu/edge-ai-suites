# How-to Guides

Follow the how‑tos to configure the system, enable the required features, and run it
successfully. The guides collected here present more details on the steps in [Get Started](./get-started.md).

- [Synchronize PTP Time (IEEE 802.1AS)](./how-to-guides/configure-ptp-gptp.md)
- [Configure the MOXA TSN Switch](./how-to-guides/configure-moxa-switch.md)
- [Configure VLAN on MOXA Switch](./how-to-guides/configure-vlan-on-moxa-switch.md)
- [Create VLAN on All Machines](./how-to-guides/create-vlan-on-all-machines.md)
- [Run RTSP Camera Capture and AI Inference](./how-to-guides/run-rtsp-camera-and-ai-inference.md)
- [Run the Sensor Data Producer](./how-to-guides/run-sensor-data-producer.md)
- [Run the MQTT Aggregator and Visualization](./how-to-guides/run-mqtt-aggregator-and-visualization.md)
- [Run the Traffic Injector](./how-to-guides/run-traffic-injector.md)
- [Enable TSN Traffic Shaping (IEEE 802.1Qbv)](./how-to-guides/enable-tsn-traffic-shaping.md)

<!--hide_directive
:::{toctree}
:hidden:

./how-to-guides/configure-ptp
./how-to-guides/configure-moxa-switch
./how-to-guides/configure-vlan-on-moxa-switch
./how-to-guides/create-vlan-on-all-machines
./how-to-guides/run-rtsp-camera-and-ai-inference
./how-to-guides/run-sensor-data-producer
./how-to-guides/run-mqtt-aggregator-and-visualization
./how-to-guides/run-traffic-injector
./how-to-guides/enable-tsn-traffic-shaping

:::
hide_directive-->